...
class MyQwen3MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.shared_expert = MyQwen3MLP(config)
    ...
    def forward(self, hidden_states, gating_reference = None) -> torch.Tensor:
        ...
        final_hidden_states = final_hidden_states.reshape(batch_size, sequence_length, hidden_dim) + shared_expert_output

        return final_hidden_states, selected_experts, router_logits
...
class MyQwen3DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.use_moe and self.gating_reference == "oracle":
            hidden_states = self.mlp(hidden_states, attn_output_wo_res)
        else: # switch
            hidden_states = self.mlp(hidden_states)
        expert_labels, router_logits = None, None
        if isinstance(hidden_states, tuple):
            hidden_states, expert_labels, router_logits = hidden_states
...
class Qwen3Model(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config):
        ...
        # print(f"Embedding params: {sum(p.numel() for p in self.embed_tokens.parameters()) / 1e9:.4f}B")
        self.layers = nn.ModuleList(
            [MyQwen3DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        # for i, layer in enumerate(self.layers):
        #     print(f"Layer {i} params: {sum(p.numel() for p in layer.parameters()) / 1e9:.4f}B")
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        output =  CausalLMOutputWithPast(
            loss=aux_loss if labels is None else loss,
            logits=logits,
        ...
        # if self.use_moe:
        #     print("output aux loss")
        #     output.loss = aux_loss
