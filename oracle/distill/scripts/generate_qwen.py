from transformers import AutoModelForCausalLM, AutoTokenizer, AutoConfig
import torch
from models import MyQwen3ForCausalLM # 这里还需要 models/myqwen.py, qwen3config.py 文件



# MoE 的参数，目前训练的 2.8B-0.8B 模型的参数就是如下
USE_MOE = True
MOE_INTERMEDIATE_SIZE = 1536
EXPERT_PER_TOKEN = 2
NUM_EXPERTS = 16
GATING_REFERENCE = "oracle" # oracle / switch

if GATING_REFERENCE == "oracle":
    PRETRAIN_CHECKPOINT = "0.18000.pth" # test checkpoint
elif GATING_REFERENCE == "switch":
    PRETRAIN_CHECKPOINT = "0.13000.pth" # test checkpoint

DEVICE = 0
CONFIG_DIR = "../../Qwen-family/Qwen3-0.6B" # 就直接用 qwen3 的 dir 就可以
CKPT_DIR = f"../checkpoints/qwen/MyPretrain-Qwen3-0.8B-2.8B-{GATING_REFERENCE}"#-from-{PRETRAIN_CHECKPOINT}" # checkpoint dir
TEST_CHECKPOINT = "0.18000.pth" # test checkpoint
DATA_DIR = "../../washed_data_0520/train" # 这里是我用的 test data，下游任务测试应该不需要。
# 1000: 偶尔能分点说
# 3000: 能分点，不会分段
# 5000: 能稳定分段分点说话
# 8000: 能稳定分点说，说的质量提高（相关性、完整性、一致性等）。
D_TYPE = torch.bfloat16
# 10：10% 的时候能输出答案。
# 30：50% 的时候能输出答案
# 50：80% 的时候能输出答案
# 80：90% 的时候按格式输出。
# 500: 100% 按格式输出。


# Question: Which one of the following is the most appropriate definition of a 99% confidence interval? A. 99% of the time in repeated samples, the interval would contain the true value of the parameter B. 99% of the time in repeated samples, the interval would contain the estimated value of the parameter C. 99% of the time in repeated samples, the null hypothesis will be rejected D. 99% of the time in repeated samples, the null hypothesis will not be rejected when it was false Answer:
# Instruction: I'm going on a trip to the US to attend a computer science conference. This is my first time visiting the US, and I'm excited to learn about the latest advancements in AI and machine learning. I hope to meet some of the leading experts in the field and gain insights that I can bring back to my work. How would you recommedn me to prepare for my trip? Answer:

@torch.no_grad()
def prepare_model():
    weights = torch.load(f"{CKPT_DIR}/{TEST_CHECKPOINT}", weights_only = True, map_location="cpu")

    config = AutoConfig.from_pretrained(CONFIG_DIR, trust_remote_code = True)
    config.moe_intermediate_size = MOE_INTERMEDIATE_SIZE
    config.num_experts_per_tok = EXPERT_PER_TOKEN
    config.num_experts = NUM_EXPERTS
    config.gating_reference = GATING_REFERENCE
    config.norm_topk_prob = True
    config.use_moe = USE_MOE

    print("Constructing student model ...")

    model = MyQwen3ForCausalLM(config).to(D_TYPE).to(DEVICE)
    model.eval()
    model.load_state_dict(weights)

    print(f'rank {DEVICE} student model ok, params: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e9:.2f}B/{sum(p.numel() for p in model.parameters()) / 1e9:.2f}B') # 
    return model


def generate_main(model, tokenizer):
    # 初始化聊天历史
    chat_history_ids = None

    print("🤖 小助手已启动！输入 'exit' 来退出对话。")

    i = 0
    while True:
        # 获取用户输入
        if i < 10:
            # user_input = "Question: Which one of the following is the most appropriate definition of a 99% confidence interval? A. 99% of the time in repeated samples, the interval would contain the true value of the parameter B. 99% of the time in repeated samples, the interval would contain the estimated value of the parameter C. 99% of the time in repeated samples, the null hypothesis will be rejected D. 99% of the time in repeated samples, the null hypothesis will not be rejected when it was false Answer:"
            user_input = "Instruction: I'm going on a trip to the US to attend a computer science conference. This is my first time visiting the US, and I'm excited to learn about the latest advancements in AI and machine learning. I hope to meet some of the leading experts in the field and gain insights that I can bring back to my work. How would you recommedn me to prepare for my trip? Answer:"
            i += 1
        else:
            user_input = input("You: ")
            if user_input.lower() in ["exit", "quit"]:
                print("👋 再见！")
                break

        # 对用户输入进行编码
        new_input_ids = tokenizer.encode(
            user_input + tokenizer.eos_token, return_tensors="pt"
        ).to(DEVICE)

        # 拼接之前的聊天历史
        chat_history_ids = None
        chat_history_ids = (
            torch.cat([chat_history_ids, new_input_ids], dim=-1)
            if chat_history_ids is not None
            else new_input_ids
        )

        # 生成回复
        chat_history_ids = model.generate(
            chat_history_ids,
            max_new_tokens = 256,          # 控制最大长度
            pad_token_id=tokenizer.eos_token_id,
            no_repeat_ngram_size=3,   # 防止重复
            do_sample=True,           # 启用采样生成更自然的回答
            top_k=30,                # Top-k 采样
            temperature=0.7,          # 控制生成多样性
        )

        # 解码并打印回复
        response = tokenizer.decode(
            chat_history_ids[:, new_input_ids.shape[-1]:][0],
            skip_special_tokens=True
        )
        print(f"Bot: {response}")


if __name__ == "__main__":
    model = prepare_model()
    tokenizer = AutoTokenizer.from_pretrained(CONFIG_DIR, trust_remote_code=True)    

    generate_main(model, tokenizer)

