...
from Dataset_new import RestaurantForLM_small, MixedData, MixedData_stage1, ACLForLM,old_MixedData_after_stage1, Mixdata_1103, Wikitext,ACLForLM_1103,RestaurantforLM_1103
...
def train(model, num_epochs, dataset, device,ahead1 ,ahead2):
    ...
    writer = SummaryWriter('1112-twany')
    ...
    torch.save(model,'1112-twany-stage0.pth')
    

if __name__ == "__main__":
    set_seed(45)
    
    config = BertConfig.from_json_file('config/bert-tawny-1112.json')
    # dataset = RestaurantForLM_small(config=config)
    dataset = RestaurantforLM_1103(config = config)
