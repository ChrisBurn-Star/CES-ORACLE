...
from Dataset_new import RestaurantForLM_small, MixedData, MixedData_stage1, ACLForLM,old_MixedData_after_stage1, Mixdata_1103, Wikitext,ACLForLM_1103
...
def train(model, num_epochs, dataset, device,ahead1 ,ahead2):
    ...
    writer = SummaryWriter("bert-1109-stage0/" + '1109-bert-wiki-stage2')
    ...
    for epoch in range(num_epochs):
        ...
        accelerator.print(f'Epoch:{epoch} ({i} Updates), Train Loss: {loss_train}, , Ahead Valid Loss: {loss_valid2}, Ahead Valid Loss: {loss_valid3}')

        if accelerator.is_local_main_process:
            writer.add_scalar('perplexity_train_epoch', loss_train, epoch)
            writer.add_scalar('perplexity_valid', loss_valid, epoch)
            writer.add_scalar('perplexity_valid', loss_valid2, epoch)
            writer.add_scalar('perplexity_valid', loss_valid3, epoch)
    ...
    torch.save(model,'1109-bert-wiki-stage2.pth')
...
if __name__ == "__main__":
    ...
    train(model=model, num_epochs=50, dataset=dataset, device=device,ahead1=ahead1, ahead2=ahead2)