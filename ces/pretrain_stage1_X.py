...
def train(model, num_epochs, dataset, dataset_pre):
    ...
    writer = SummaryWriter("log-1021-TOORIGIN-pca/" + 'bert')
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_loader):    
            ...
            for j, (detached_output, standard_pca) in enumerate(zip(detached_outputs, standard_pcas)):                
                ...
                if not use_pca:
                    pca_loss = mse_loss(detached_output, standard_pca[epochp * config.batch_size : (epochp+1) * config.batch_size]) 
                else:
            ...
            # del scores, detached_scores, score_loss, pca_loss, loss, local_optimizer, memory, layer_outputs, detached_outputs, detached_output, batch, batch_old, pca_old
    ...
    accelerator.save_state('./output-formal-2-X-1021-TOORIGIN-pca')
    torch.save(model, './output-increamental/1021-TOORIGIN-pca.pth')
