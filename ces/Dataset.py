...
class GLUE():
    def __init__(self, config) -> None:
        ...
        self.test_loader = DataLoader(tokenized_datasets['test'], batch_size=self.batch_size, shuffle=True)
...
class SST2_w():
    def __init__(self, config) -> None:
        ...
        # print(11,datasets["train"][0])
        W_ids1 = torch.load('0416-W_IDS-sst2-T.pth')
        W_ids2 = torch.load('0416-W_IDS-sst2-V.pth')
        ...
        # print(tokenized_datasets['train']['w_ids'])
        # tokenized_datasets.save_to_disk('/home/jxzhou/PLM_PER/finetunedatasets/SST2_w')

        # tokenized_datasets['train'] = torch.utils.data.Subset(tokenized_datasets['train'], range(10000))
...
if __name__ == "__main__":
    ...
    dataset = SST2_w(config)
    # agnews = AGNews(config)
    # cifar10 = Cifar10()

    # config = BertConfig.from_json_file('config/bert_glue.json')
    # dataset = GLUE(config)