...
def train(model, num_epochs, dataset, device, ahead_dataset):
    ...
    model = torch.load('bert-1030-stage1.pth')
    ...
    writer = SummaryWriter("bert-1030-stage2/" + 'bert')
    ...
    for epoch in range(num_epochs):
        ...
        ahead_loss_valid = validate(model, ahead_val_loader, accelerator, device)
        ...
        accelerator.print(f'Epoch:{epoch} ({i} Updates), Train Loss: {loss_train}, Valid Loss: {loss_valid}, Ahead Valid Loss:{ahead_loss_valid}')
    ...
    accelerator.save_state('./bert-1030-stage2')
    torch.save(model,'bert-1030-stage2.pth')
