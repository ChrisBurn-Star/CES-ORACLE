...
class TransformerEncoder(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.moe = nn.ModuleList([FeedForward(config) for _ in range(config.num_experts)])
        
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.register_buffer('cluster_centers', torch.zeros(config.num_experts, config.hidden_size))

    def set_cluster_centers(self, new_centers):
    ...
    def route(self, x):
        with torch.no_grad():
            ...
            dist = torch.norm(x.unsqueeze(1)-self.cluster_centers, dim=-1)
            routes = torch.argmin(dist, -1)
        return routes
    ...
    def forward(self, hidden_states, attention_mask) -> torch.Tensor:
        ...
        routes = self.route(att_output)
        indexes_list = [torch.eq(routes, i).nonzero(as_tuple=True)[0] for i in range(self.cluster_centers.shape[0])]
        expert_output = [self.moe[i](att_output[indexes_list[i], :]) for i in range(self.cluster_centers.shape[0])]
        for i in range(self.cluster_centers.shape[0]):
            final_output[indexes_list[i], :] = expert_output[i]

        ffn_output = self.dropout(final_output)
...
class TransformerEncoders(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask) -> torch.Tensor:
        ...
        for i, layer_module in enumerate(self.layers):
            routes.append(layer_module.route(hidden_states))
            hidden_states = layer_module(hidden_states, attention_mask)
        return hidden_states, routes
