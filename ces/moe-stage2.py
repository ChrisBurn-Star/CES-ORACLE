...
def train(model, num_epochs, dataset, device, ahead_dataset):
    ...
    baseline_data = load_layer_data('1120-baseline_data_rose_models-2experts.pth')
    ...
    writer = SummaryWriter('1127_vermilion_model_stage2_1.5e-4')
        ...
        for i, batch in enumerate(train_loader):
            ...
            if baseline:
                if i%REPLAY_STEPS == 0:
                    for c in IDS:
                        if len(test[c]):
                            for data in baseline_data[0]:
                                replay_loss, _, _ ,_, _= model(data['input_ids'].view(1,config.seq_len),data['attention_mask'].view(1,config.seq_len), data['labels'].view(1,config.seq_len), cluster_centers)
                                optimizer.zero_grad()
    ...
    torch.save(model,'1127_vermilion_model_stage2_1.5e-4.pth')
