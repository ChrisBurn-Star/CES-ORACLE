...
def validate(model, val_loader, accelerator,center_model,cluster_centers,lora_mat):
    losses = []
    ...
    for i, batch in enumerate(val_loader):  
        ...
        with torch.no_grad():   
                ...
                hidd,_ = center_model.bert(batch['input_ids'],batch['attention_mask'])
                hidd = hidd.mean(1)
        ...
        with torch.no_grad():
            ...
            loss, _= model(batch['input_ids'],batch['attention_mask'], batch['labels'], ws)
...
def train(model, num_epochs, dataset, device,LEGAL):
    ...
    cluster_centers = load_layer_data('0518-layer_centers-att-4t-longtail.pth')
    ...
    writer = SummaryWriter('/home/jxzhou/PLM_PER/BERT-CL-main/rebutall_0603/mof-copyparams')
    ...
    # load_checkpoint_and_dispatch(center_model, 'MODELS_0429/0429-BERT-768ffns-wikipedia-1.5e-4',device_map="auto") 
    ...
    #         model.layers[i].expert.ffn.load_state_dict(model0.bert.encoders.layers[i].ffn.state_dict())
    #         for j in range(config.num_transformer):
    #             model.layers[i].unique_atts[j].load_state_dict(model0.bert.encoders.layers[i].attention.state_dict())
    #     else:
    #         model.layers[i].load_state_dict(model0.bert.encoders.layers[i].state_dict())
    #################   MOF    ############
    for i in range(config.num_hidden_layers):
        if i ==config.num_hidden_layers - 1 or i == config.num_hidden_layers - 2:
            model.layers[i].common_att.load_state_dict(model0.bert.encoders.layers[i].attention.state_dict())
            for j in range(config.num_transformer):
                model.layers[i].expert.unique_ffns[j].load_state_dict(model0.bert.encoders.layers[i].ffn.state_dict())
        else:
            model.layers[i].load_state_dict(model0.bert.encoders.layers[i].state_dict())
    ...
    model,optimizer, lr_scheduler, train_loader, val_loader,val_loader1,val_loader2,center_model,cluster_centers[-1],lora_mat= accelerator.prepare(model,optimizer, lr_scheduler, train_loader, val_loader,val_loader1,val_loader2,center_model,cluster_centers[-1],lora_mat)
    ...
    # y = 0
    steps = 0
    # accelerator.save_state('MODELS_0514/MOMOE-multipleffn-768ffns-mixed-small-1.5e-4-checkpoints%d'%steps)
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            #     distances = distances - cluster_radius[-1].view(-1)
            #     ws = torch.argmin(distances, dim=1)
            ...
            # loss, _ = model(batch['input_ids'],batch['attention_mask'], batch['labels'],batch['w_ids'])
            loss, _ = model(batch['input_ids'],batch['attention_mask'], batch['labels'],ws)
            ...
            if steps%100 == 0:
                ...
                loss_valid = validate(model, val_loader, accelerator,center_model,cluster_centers,lora_mat)
                loss_valid1 = validate(model, val_loader1, accelerator,center_model,cluster_centers,lora_mat)

                loss_valid2 = validate(model, val_loader2, accelerator,center_model,cluster_centers,lora_mat)
            ...
            # if steps%800 == 0:
            #     accelerator.save_state('MODELS_0515/MOMOE-new3-multipleatt-KMEANS-768ffns-mixed-large-1.5e-4-checkpoints%d'%steps)
    ...
    accelerator.save_state('/home/jxzhou/PLM_PER/BERT-CL-main/MODELS_REBUTALL/mof-copyparams')
...
if __name__ == "__main__":
    ...
    # dataset = Wikipedia_small(config = config)
    LEGAL = 100000
    ...
    model = base_models.MoMoE_0514_2(config=config)

    


    
    train(model=model, num_epochs=3, dataset=dataset, device=device,LEGAL=LEGAL)
