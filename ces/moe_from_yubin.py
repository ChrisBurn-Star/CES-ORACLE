...
def train(model: BertForMLM, dataset, epoch):
    ...
    if accelerator.is_local_main_process:
        writer = SummaryWriter('/home/jxzhou/PLM_PER/BERT-CL-main/rebutall_0603/moe-copyparams-med-specific')
    ...
    #         model.bert.encoder.layers[i].attention.load_state_dict(model0.bert.encoders.layers[i].attention.state_dict())
    #         for j in range(config.num_experts):
    #             model.bert.encoder.layers[i].ffn.experts[j].load_state_dict(model0.bert.encoders.layers[i].ffn.state_dict())
    #     else:
    #         model.bert.encoder.layers[i].load_state_dict(model0.bert.encoders.layers[i].state_dict())
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_loader):
            steps += 1
            # print(i)
            # print(batch['attention_mask'][:, None, None, :].shape)
    ...
    accelerator.save_state('/home/jxzhou/PLM_PER/BERT-CL-main/MODELS_REBUTALL/moe-copyparams-med-specific')

def validate(model: BertForMLM, val_loader, accelerator):
    losses = []
    for i, batch in enumerate(val_loader):
        with torch.no_grad():
            loss, loss_dict,_,_= model(**batch)
