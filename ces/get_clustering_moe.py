...
class BertForMLM_every2layers(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels):
        output,att_sself,O,_,FFN_E,OUT_E = self.bert(input_ids, attention_mask)
...
def show_clusters(inputs,l,name):
    ...
    # plt.scatter(X[-600:-480,0],X[-600:-480,1],color = "brown")
    # plt.scatter(X[-480:-360,0],X[-480:-360,1],color = "b")
    plt.scatter(X[-360:-240,0],X[-360:-240,1],color = "r")
    ...
    plt.savefig("0531-cluster-moe-name%s-l%d.png"%(name,l))
...
def load_layer_data(path):
    ...
    return layer_data
def layer_pca(model, datasets,STEP):
    out_for_cluster = [[] for i in range(12)]
    out_e_for_cluster = [[[]for j in range(4)] for i in range(2)]
    ffn_e_for_cluster = [[[]for j in range(4)] for i in range(2)]
    ...
    for i in datasets:
        ...
        load_checkpoint_and_dispatch(model, '/home/jxzhou/PLM_PER/BERT-CL-main/MODELS_0514/MOE-768ffns-mixed-small-1.5e-4-checkpoints%d'%STEP,device_map={"": device}) 
        ...
        model, optimizer, lr_scheduler, train_loader,lora_mat = accelerator.prepare(model, optimizer, lr_scheduler, train_loader,lora_mat)
        ...
        with torch.no_grad():
            for i, batch in enumerate(train_loader):
                ...
                print(f"######{i}")                
                _,_,_,_,FFN_E,OUT_E = model.bert(batch['input_ids'],batch['attention_mask'])
                # _,_,outputs,_ = model.bert(batch['input_ids'],batch['attention_mask'])
                # outputs = torch.matmul(outputs.mean(1),lora_mat.to(outputs.device))

                # for j in range(12):  
                #     out_for_cluster[j].append(outputs.detach().view(outputs.shape[0],-1))
                for j in range(2):  
                    for k in range(4):
                        out_e_for_cluster[j][k].append(OUT_E[j][k].detach().view(OUT_E[j][k].shape[0],-1))
                for j in range(2):  
                    for k in range(4):
                        ffn_e_for_cluster[j][k].append(FFN_E[j][k].detach().view(FFN_E[j][k].shape[0],-1))
    ...
    #     show_clusters(out_for_cluster[j],j)
                        
    for j in range(2):  
        for k in range(4):
            ffn_e_for_cluster[j][k] = torch.cat(ffn_e_for_cluster[j][k], dim =0 ).detach()
    for j in range(2):  
        for k in range(4):
            out_e_for_cluster[j][k] = torch.cat(out_e_for_cluster[j][k], dim =0 ).detach()
    for j in range(2):

        show_clusters(out_e_for_cluster[j],j,"out")
        show_clusters(ffn_e_for_cluster[j],j,"ffn")
...
if __name__ == "__main__":
    ...
    layer_pca(model=model, datasets=datasets,STEP = 7000)
